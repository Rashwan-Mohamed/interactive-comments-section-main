...
export const Replay = ({ id, content, createdAt, score, user, replyingTo }) => {
  const { toggle, currentUser, deleteIt, editIt, replay, comments } =
    useGlobalContext()
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    editIt({ id, good: true, content: comment })
    setIsEditing(false)
  }
    ...
    <>
      <article className='feedback' key={id}>
        <section className='fore'>
          <Teoggle id={id} score={score} good={true}></Teoggle>
          ...
          {currentUser.username === username ? (
            <div className='itsme'>
              <button onClick={() => deleteIt({ id, good: true })}>
      ...
      {isReplaying && (
        <article className='feedback addIt resite'>
          {' '}
